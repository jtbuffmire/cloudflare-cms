...
export async function updateSiteConfig(
  ...
  try {
    ...
    const parsedHeaders = updatedRow.about_section_headers ? JSON.parse(updatedRow.about_section_headers as string) : [];
    const parsedContents = updatedRow.about_section_contents ? JSON.parse(updatedRow.about_section_contents as string) : [];
    const parsedAboutSections = parsedHeaders.map((header: any, i: number) => ({
      title: header.title,
      visible: Boolean(header.visible),
      content: parsedContents[i]?.content?.text ?? ''
    })) || [];
