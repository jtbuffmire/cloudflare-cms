...
import type { Post, PicsItem, SiteConfig, CreatePostData, CreatePicsData, UpdateSiteConfigData } from '$lib/types';
...
export class Database {
  ...
  async getPicsItems(): Promise<PicsItem[]> {
    const { results } = await this.db
      .prepare('SELECT * FROM pics ORDER BY created_at DESC')
      .all();
    return (results as D1Result[]).map((row) => ({
      id: String(row.id),
  ...
  async createPicsItem(pics: CreatePicsData): Promise<PicsItem> {
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
